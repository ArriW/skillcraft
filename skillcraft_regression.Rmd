...
```{r ord_confusion, echo=FALSE}
conf <- table(Actual = y_true, Predicted = factor(pred_ord, levels = 1:7))
kable(as.matrix(conf),
      caption = "$ord_{final}$ confusion matrix (rows = actual, cols = predicted)")
```

```{r ord_confusion_plot, echo=FALSE}
ord_pred_df <- data.frame(LeagueIndex = y_true, Pred = pred_ord) %>%
  group_by(LeagueIndex) %>%
  summarise(correct_perc = mean(Pred == LeagueIndex),
            within1_perc = mean(abs(Pred - LeagueIndex) <= 1),
            baserate     = n() / length(y_true)) %>%
  reshape2::melt(id = "LeagueIndex")

ggplot(ord_pred_df, aes(x = factor(LeagueIndex), y = value, fill = variable)) +
  geom_bar(stat = "identity", width = .6, position = "dodge") +
  theme_classic() + ylab("Proportion") + xlab("LeagueIndex") +
  ggtitle(paste0("ord_final accuracy by LeagueIndex (overall acc = ",
                 round(acc(pred_ord, y_true) * 100), "%, within-1 = ",
                 round(within1(pred_ord, y_true) * 100), "%)"))
```

## Takeaways from the Ordinal Refit

With the right tooling the ordinal model is no longer the embarrassing tail of this analysis. Three observations worth calling out:
