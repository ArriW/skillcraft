...
![https://blizzcon.com/en-us/news/23198508/congrats-to-the-starcraft-ii-wcs-global-finals-champion](WCS.png)

  The eye watering speeds or more specifically in gaming terminology the action per minute (APMS) of these players was visibly remarkable. Professionals make actions at such fastspeeds it becomes challenging to follow their overall stategy. SI found it difficult to distinctly define what made these players professionals past their raw agility.

To learn more about what defines talent in SC2 this analysis we will explore in game metrics in attempts to predict the response League Rank refered in in this analysis as LeagueIndex.

The actual source of the data was cleaned and prepared by :
https://archive.ics.uci.edu/ml/datasets/SkillCraft1+Master+Table+Dataset

## Goal

To model the response LeagueIndex we will explore data from a 2013 ranked season of SC2. The predictors provided summarize ingame performace metrics for the season  per player (GameID). 
...
# Data Exploring In Game Metrics
...
There is a strong depature from normality for the hous per week predictor
```{r,echo=FALSE}
...
ggplot(sc, aes(scale(log(HoursPerWeek)))) +
  geom_density()+
  ggtitle(
    'Log:Hour per week distribution',subtitle = paste(HoursPerWeek_Normal[3],
    " P-Value: ",HoursPerWeek_Normal[2]))+
  xlab("Hours Per Week")+
  ylab("Count")

qqnorm(scale(sc$HoursPerWeek))
qqline(scale(sc$HoursPerWeek))
```
```{r}
qqnorm(scale(sc$APM))
```
...
sc_lm_1_sum<-summary(sc_lm_1)
sc_lm_9_sum<-summary(sc_lm_9)
```

The intial model starts is of the form $LeagueIndex=X\beta$,where n = {3337} and p = {18}. Predictors were trimmed by least signifance until all parameters were signficant at an $\alpha=5\%$. The resulting $X$ had p={9}.

The ANOVA table below evaluates the _____ of the models. Risidual error? After removing 5 predictors the difference in the residuals was signficant. The  $adjustedR^2$ difference was mininamal starting at `r round(sc_lm_1_sum[["adj.r.squared"]],2) ` and ending at `r round(sc_lm_9_sum[["adj.r.squared"]],2) `.

The W statistic `r shapiro.test(residuals(sc_lm_9))`

```{r}
anova(sc_lm,sc_lm_1,sc_lm_2,sc_lm_3,sc_lm_4,sc_lm_5,sc_lm_6,sc_lm_7,sc_lm_8,sc_lm_9)
summary(sc_lm_9,cor='T')
qqnorm(residuals(sc_lm_9))
qqline(residuals(sc_lm_9))+title("hello")
```
