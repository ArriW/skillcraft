...
git add --all
git commit -m "Cleaned up whole document from mac"
git push
```
...
The some of the time averaged metrics are per SC2 timestamps while other are per milisecond. To make these metrics more interperatable each metric will be converted into seconds. There are roughly 88.5 timestamps per second so each metrix in timestamps will be multiplied by 88.5 [https://archive.ics.uci.edu/ml/datasets/SkillCraft1+Master+Table+Dataset].
