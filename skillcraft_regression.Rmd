...
git add --all
git commit -m "Tests of confounding"
git push
```
...
Assign to hotkeys is a fairly highly correlated with APM that they warrant a further investigation. **AssignToHotkeys** was considered to be dropped along with the PAC related predictors prior but I imaged it added a significant flavor to what type of actions regardless of its potential confoundince.
```{r}
sc_lm_5<-update(sc_lm_final,.~.+ComplexUnitsMade)
kable(tidy(sc_lm_5,conf.level = .05)[9:10,],caption = "Confounding Model CompelxUnitsMade and Complex Abilities Used")
kable(anova(sc_lm_final,sc_lm_5),digits=2)
```

The notable change in **ComplexAbilitiesUsed** is is likely a result the removal of the variable **ComplexUnitsMade**. Upon reintroducing ComplexUnitsMade into the final model we find the following p-values for both predictors to be insignificant. This is a warning that we may not be able to distriguish the effects or vary them independently. This aligns with what is exepcted, players have to make complex units before they can use their compelx abilities.
```{r}
sc_lm_5<-update(sc_lm_final,.~.+ComplexUnitsMade)
kable(tidy(sc_lm_5,conf.level = .05)[9:10,],caption = "Confounding Model CompelxUnitsMade and Complex Abilities Used")
kable(anova(sc_lm_final,sc_lm_5),digits=2)
...
In each of the 3 cases linear models with and without pieces of the pairs will be explored.
...
# References
[^ViolinPloyts]: This is a random test.