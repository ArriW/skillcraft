...
Our intial $H_o$ is there is no systematic structure to the response LeagueIndex. Our alternative $H_a$ is there is some relation such that $LeagueIndex=X\beta+\epsilon$, where X all other variables with the exception of index GameID. Without much surpise using18 predictor variable results of a very small p-value of ~`r glance(sc_lm)$p.value `. Over the interration this does not change in a notable fashion across the 9 other models as the last model results in a `r glance(sc_lm_9)$p.value`
