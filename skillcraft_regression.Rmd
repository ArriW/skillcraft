...
The question for the final research is aligned with the scope of the intial research question, and that is to explain what makes players highly ranked. The last effort was limited because it only dabbled in diagnostics but more importantly because the it applied a multivariant linear regression to a ordinal response. 

The final modeling will return to the MLR to peform some diagnostics on $lm_\omega$. Subsequently the $lm_\omega$ will be redediated based on the findings. Additionally a stepwise MLR $lm_\circlearrowleft$ will minimize Bayesian information criterion (BIC) for explaintory purposes. The last model that will be produced will utilize the same subset of predictors.

All the models will be compared how ever i can.

# Perform Diagnosistics on old model

Knowing that these $lm_\omega$ has been improperly to the ordinal response most of the diagnostics and related assumptions will be consistently violated regardless of transformations within MLR modeling. The expection to this would be the outliers levarage, by addressing these needs to iterate will be brought to the forefront. 

After using a Cook's distance plot it is apparant with a index 2321 needs adjustment. Upon further inspection this data points seems to be another data point 

```{r}
sc_lm<-lm(LeagueIndex~.-TotalHours+log(TotalHours),sc)
sc_lm_1<-update(sc_lm,.~.-UniqueUnitsMade ,sc)
sc_lm_2<-update(sc_lm,.~.-Age-UniqueUnitsMade)
sc_lm_3<-update(sc_lm,.~.-Age-UniqueUnitsMade-ComplexUnitsMade)
sc_lm_4<-update(sc_lm,.~.-Age-UniqueUnitsMade-ComplexUnitsMade-MinimapRightClicks)
...
#Filter first outlier with high leverage. 20000 (Total Hours)
sc<-filter(sc,sc$TotalHours<25000)
sc_lm<-lm(LeagueIndex~.,sc)
sc_lm_1<-update(sc_lm,.~.-UniqueUnitsMade ,sc)
sc_lm_2<-update(sc_lm,.~.-Age-UniqueUnitsMade)
sc_lm_3<-update(sc_lm,.~.-Age-UniqueUnitsMade-ComplexUnitsMade)
...
sc<-filter(sc,sc$TotalHours<20000)
sc_lm<-lm(LeagueIndex~.,sc)
sc_lm_1<-update(sc_lm,.~.-UniqueUnitsMade ,sc)
sc_lm_2<-update(sc_lm,.~.-Age-UniqueUnitsMade)
sc_lm_3<-update(sc_lm,.~.-Age-UniqueUnitsMade-ComplexUnitsMade)
...
sc<-filter(sc,sc$TotalHours<18000)
...
- partial residuals by each predictor Lecture Model structure and gooodness of fit and for multiple morality

- 
### Revise as needed
### Generate a stepwise model here too...
