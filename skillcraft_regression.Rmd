...
git add --all
git commit -m "Further cleaning and visualizations."
git push
```
...
Videogames are one of my favorite past times. As a player who participates in ranked play, I've always kept my eye on the forefront of global competitions. One of the most notable games to establish a global competitive scene backed by paid professionals was the real-time strategy game (RTS) Starcraft II (SC2). In 2013 the top 10 starcraft players by earnings made nearly four-million dollars combined from their winnings alone @Winnings2019. Watching top-caliber players reflexes and control astonishing is even seasoned videogame enthusiasts. At the 2019 StarCraft II World Championship Finale, I and many others packed into the arena to see what these professions could do firsthand.
...
A more suitable form of model for this regression would be based off a Polytonomous Logistic Regression for Ordinal Response (Proportional Odds Model) @OrdinalLog. These methods will be revisted for the final portion of this analysis.
...
```
Finally, upon preforming a visual inspection on the data hoursperweek contains max value of 168. Considering there are 168 hours in a week its not plausible for a individual player to do this. There could be multiple players using this account making this possible. Or in a much neater prospect maybe this player is actually an AI like google's DeepMind [https://deepmind.com/blog/article/alphastar-mastering-real-time-strategy-game-starcraft-ii]. Either way this observation will be kept because what is realistic cutoff for the hours per week is not apparent and after removing the observation the next max value is 140 which seems almost as unrealistic.

Its worth noting that dropping this or a handful of values far from combats all the issues encouterable through Hours Per Week and or total hours. Multiple players could be using any of the accounts. More likely then that many of these accounts could be smurf accounts. Smurfing is when a player makes a additional accounts to have a match making rating reset [https://www.lol-smurfs.com/blog/what-is-a-smurf-account/]. A common reason for doing this is to dominate the competition until their rating adapts to their skill level.

```{r,echo=FALSE}
sc<-filter(sc,sc$HoursPerWeek<168)
```
...
- Remove GapBetweenPACs ^[Another issue with this is that its unist do not seem to align with the discription. Before and after the unit transformation GapBetweenPACs results in periods that dont make since for the expected duration of SC2 matches.)]
- Remove NumberofPACS
- Remove SelectbyHotkey
- Remove ActionsInPAC

Focusing exclusively on APMs fit into an Occam's razor approach by the models footprint.
...
Visually determining trends between the predictors and the response with a response is that ordinal requires tossing aside scatter plots for a better suited tools. In a visualization like this I prefer to utilize Violinplots in[https://chartio.com/learn/charts/violin-plot-complete-guide/}

Upon aspecting hours 
```{r}

p1<-ggplot(sc, aes(x=factor(LeagueIndex), y=age)) +
  geom_violin(trim=FALSE, fill='#A4A4A4', color="blue") + stat_summary(fun.data=mean_sdl, mult=1, 
                 geom="pointrange", color="darkblue")+ coord_flip()
p2<-ggplot(sc, aes(x=factor(LeagueIndex), y=HoursPerWeek)) +
  geom_violin(trim=FALSE, fill='#A4A4A4', color="blue") + stat_summary(fun.data=mean_sdl, mult=1, 
                 geom="pointrange", color="darkblue")+ coord_flip()
p3<-ggplot(sc, aes(x=factor(LeagueIndex), y=TotalHours)) +
  geom_violin(trim=FALSE, fill='#A4A4A4', color="blue") + stat_summary(fun.data=mean_sdl, mult=1, 
                 geom="pointrange", color="darkblue")+ coord_flip()
p4<-ggplot(sc, aes(x=factor(LeagueIndex), y=APM)) +
  geom_violin(trim=FALSE, fill='#A4A4A4', color="blue") + stat_summary(fun.data=mean_sdl, mult=1, 
                 geom="pointrange", color="darkblue")+ coord_flip()
...
```{r,echo=FALSE}
#summary(sc_lm<-lm(LeagueIndex~.,sc))
#summary(sc_lm_1<-update(sc_lm,.~.-UniqueUnitsMade ,sc))
#summary(sc_lm_2<-update(sc_lm,.~.-Age-UniqueUnitsMade))
#summary(sc_lm_3<-update(sc_lm,.~.-Age-UniqueUnitsMade-ComplexUnitsMade))
summary(sc_lm_4<-update(sc_lm,.~.-Age-UniqueUnitsMade-ComplexUnitsMade-MinimapRightClicks),cor='T')
sc_lm_5<-update(sc_lm,.~.-Age-TotalMapExplored-UniqueUnitsMade-MinimapRightClicks-ComplexUnitsMade)
...
Our second null test is to determine if the the starting and final model's have significantly different residuals. The $H_o$ specifically is that $RSS_{lm1}$ = $RSS_{lm4}$ and $H_a$ is that $RSS_{lm1}$ \neq $RSS_{lm4}$. Performing an ANOVA test we find that there is not significant difference in the models at $\alpha=5\%$ such that we cannot reject the $H_o$.(ANOVA see table below). Additionaly their $adjR^2$ is barely different. Starting at `r round(summary(sc_lm)[["adj.r.squared"]],2)` and ending at `r round(summary(sc_lm_4)[["adj.r.squared"]],2)`.
