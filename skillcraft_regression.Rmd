...
git commit -m "Starting with log transform"
git push
...
Subsequently, additional diagnostics will performed iteratively with remediation as needed ^[Four iteration of prelimary diagnostics/redimation on $lm_\omega$ revealed **TotalHours** contained four points with high levearge with Cook's Distance greater than 1. To prevent this analysis from balloning due to excessive remediation a log transform on **Totalhours** will be performed prior to creating $lm_\circlearrowleft$, while $lm_\omega$ will be left the same to provide a stark constant between the intial naive appoarch and more formal model selection.].

The last model $ord_\circlearrowleft$ will use ordinal regression with the same predictors as $lm_\circlearrowleft$ to provide a model fitted to the ordinal nature of the respionse while keeping selection of predictors streamline. Finally the 3 models will be compared using BIC and predictive power.

? Does anyone ave 0 total hours?
```{r logtransform ,include=FALSE}
sc_c<-sc_n%>%mutate(TotalHours=log(TotalHours))
```

A comparision of $lm_\circlearrowleft$ and $lm_\omega$ will happen.

To provide an analysis more fitted to the response, an ordinal regression will preformed using the same predictors as $lm_\circlearrowleft$ . ^[The same set of predictors will be used mainly because I lack a package to do stepwise ordinal regression.]. Finally  these models will be compared using BIC and their predictor power.
...
# Perform Diagnosistics on old model

Knowing that $lm_\omega$ has been uncharecistically fitted to the ordinal response most of the diagnostics and related assumptions will be consistently violated regardless of transformations within MLR modeling. The expection to this would be the outliers levarage, by addressing these needs to iterate will be brought to the forefront.
...
### APM in the ordinal model

### (Maybe) Diagnositics on Proportional odds model
...
```{r}
#pc<-sc%>%group_by(LeagueIndex)%>%summarize(AvgAPM=mean(APM))
```
...
## MLR vs Proportional Odds Model 
- Briefly Compare the previous model and new models power using the same predictors, because the following models analyzed may not be a subset of one another.
- Revisit model specifications with a step wise reg focused on BIC
- Predict Power
