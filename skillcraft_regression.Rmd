...
git add --all
git commit -m "Exploring the visual relationships and definining the natrue"
git push
```
...
**HourPerWeek** The square root of HoursPerWeek has visibly little or no relation to LeagueIndex 1-4 where LeagueIndex 4-7 seems to have a strong linear trend resulting in `r cor_hoursperweek`. If HoursPerWeek survives the model trimming, it's bimodality may cause issues w model assumption $COV(Y)=\sigma^2I$.

**TotalHours** The square root of HoursPerWeek positive linear trend with the response.

**APM** has a strong linear relationship with the response. 

**AssignToHotkeys** Has a notable fractional relationship with the response. This also may cause issues with the Gaussianity of the error.
...
do.call("grid.arrange", c(plots[5:9], ncol=2))
do.call("grid.arrange", c(plots[10:13], ncol=2))
```
