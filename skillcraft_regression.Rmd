...
git commit -m "Sedona final, almost changed everything"
git push
...
Perform diagnostic analysis of your model. You should check for our usual assumptions: evaluate the structure of the model (e.g., nonlinearity in the relationship, residual versus fitted and partial residual plots);
        ...
        -(EMAIL COLIN)(NOT INCLUDED) Compute the k-fold cross validation RMSE to evaluate a more realistic estimate of predictive performance.

    Revise the model and perform remediation of the issues: (CHECK EACH OF THESE OFF IN REFERENCE TO FIRST PAPER AS NEEDED)
        -(EMAIL COLIN)(DONE IN MIDTERM2?, what about log transform introducing 0s? Do i need to do anything more through)if reasonable to do so, handle missing data systematically with methods from class;
        (This will be done with BIC plots )compare the model selection in the midterm with model instead based upon an information criterion method;
        (Logit?) if reasonable to do so, consider transformation of scale of the response with e.g., Box-Cox, shifted log, logit or Fisher’s z-transformation;
        (EMAIL COLIN if I need to redo remediation done in first part) (DONE, in Midterm2 log)if reasonable to do so, exclude outliers and re-fit the model;
        (WHAT is general least squares)if reasonable to do so, perform generalized or weighted least squares (generalized least squares is particularly difficult in most cases, so unless you know the covariance structure of the errors already, don’t worry about estimating it or performing GLS);
       ...
       (I got this covered) Not all of the above will be reasonable to perform with your data. Some of the above remediation (like outlier exclusion) depend on the other remedial steps (like variable transformation) and should be considered simultaneously. You are encouraged to iterate on this several times, but only to summarize your process on how you arrived at your final version of the model, and how (or if) it has addressed the issues noted in the diagnostics.
   (Goodness of Fit? Past that I need to redo my Omega/omega with the addition of BIC) Compare the goodness of fit, uncertainty, explanatory and predictive power of the new model with the model selected in the midterm. The above model evaluations should follow the same guidelines and expectations of the midterm. If you have arrived at the same model, use advanced tools such as partial residual plots to discuss the structural uncertainty of the model. Expand upon the earlier analysis to include comparisons of the effects of parameters based on, e.g., change of scale or exclusion of influential observations. Discuss the differences in predictive power observed with the confidence intervals and with the cross validation RMSE.
...
The question for the final research is aligned with the scope of the intial research question, and that is to explain what makes players highly ranked. The last effort was limited because it adhocly reduced the span of the model using p-values, only dabbled in diagnostics, but more importantly because the it applied a multivariant linear regression to a ordinal response. 

To select better combination of predictors, the final analysis once again utilizes MLR but instead in combination with stepwise regression. The stepwise regression $lm_\circlearrowleft$ provides an more exhaustive examination of the combination of predictors then the intial $lm_\omega$. With $lm_\omega$ many predictors were trimmed prior to modeling based on the concern that they may be colinear APMs thus hendering the explaintory power of the model. These predictos will be reinstated for $lm_\circlearrowleft$'s selection because the Bayesian information criterion (BIC) should account for the variance inflation and trim accordingly. Should any of these predictors survive the trimming, they will bere-examined. 

Subsequently, additional diagnostics will performed iteratively with remediation as needed ^[Four iteration of prelimary diagnostics/redimation on $lm_\omega$ revealed **TotalHours** contained four points with high levearge with Cook's Distance greater than 1. To prevent this analysis from balloning due to excessive remediation a log transform on **Totalhours** will be performed prior to creating $lm_\circlearrowleft$, while $lm_\omega$ will be left the same to provide a stark constant between the intial naive appoarch and more formal model selection. **TotalHours** does not contain any 0 values.].
...
Choosing the best model provided by five predictors intuitatvely seemed like a good route for explaintory purposes as a "Top 5 Skills for aspiring starcraft players" sounds like a good starting point, but demishing returns concerning BIC also seems to support 5 as a reasonable cutoff.

# Diagonostics

Knowing that $lm_\omega$ has been uncharecistically fitted to the ordinal response most of the diagnostics and related assumptions will be consistently violated regardless of transformations within MLR modeling

## Confounding

It was was a intial concern that **ActionLatency** and **TotalHours** would be colinear. The variance infliation factor is not very large ?How do a I interpret VIF. BIC differnce is low and corr is high? , but the size of the $Cor(ActionLatency,TotalHours)$=`r cor(sc_n$APM,sc_n$ActionLatency)` is cause for concern. 

To further explore the impact of these two variables. The $update()$ function is used to remove each variable one at a time to compare the effects of dropping them respectively to the original model. 

Make table to summarize ( adjusted^2 , BIC, Confidence interval) of the 3 models... The impact of dropping APMs seems weak
```{r, echo=FALSE}
...
In midterm 2 tests for normality were performed on the ordinal response **LeagueIndex** and the residuals of $lm_\omega$, both were found have have nongaussian residuals. The same holds true for $lm_\circlearrowleft$. This prevents the our regression models from being the maxuimum likihood estimators and convoludes applicapations of ANOVA. The generation of an ordinal model is the most appropiate way to proceed, but to exercerise the requirements for final report the remainder of the diagnosisitcs try to address other problems concerning linear regression model assumptions.
...
The Residuals vs Preictors/fitted plots reveals some odd behavior around the tails. On the left there are scarse amount of fitted values with some being outside the possible range of **LeagueIndex**. Overall the left end of the tail is biased to over estimate. On the right there are also a scarse amount of fitted values with some greater then the possibilities for **LeagueIndex** with some biase to un

$lm_\omega$ fitted values have a very long right tail that places players with a **LeagueIndex** greater than Grandmaster (**LeagueIndex**=seven)  and a short left tail that does not place any player lower silver (**LeagueIndex**=2). At both tails the model is bias to grearly overestime while center quartiles tend to more subtely underestimate. Upon closer examination this bias could be driven by **TotalHours, APM, MiniMapAttacks, WorkersMade, and ComplexUnitsMade**. It might be reasonable to project that this model would suffer from nonnormal residuals even if the response was continous.

```{r, echo=FALSE}
residualPlots(sc_lm_final)
hist(fitted(sc_lm_final))
```
On the otherhnad $lm_\circlearrowleft$ visually performs much better than $lm_\omega$ as the fitted vs resiudal amoothed line seems to have intercept and slope approximately 0. The log transformation performed on **TotalHours** appears to have medigated the trend between residuals and the predictor's values. **ActionLatency, AssigntoHotkeys, APM, and MinimapAttacks** have trends with their respective values and the residuals. Transformation on these predictos were considered but not taken because it would complicate their explaintory power and the transforms would do little to medigate the issues of applying a linear model to an ordinal value.

```{r, echo=FALSE}
residualPlots(sc_sw_lm)
hist(fitted(sc_sw_lm))
```

The residuals vs ditted 
NormalQQ:
...
The following uses BIC to compare the iterative ?backward model selection based on trimming the least significant p-value seen in midterm 2 and the second based an exhaustive stepwise regression for all possible size combinations.

Overlap between the two series concerning BIC is not expected  because the first series, based on pvalues, had the ?5 potentially colinear variables trimmed before any modeling was conducted while the stepwise regression starts with one predictor and finishes with the inclusion of all predictors. ?another differnece is the log transforms
...
Construct a ordinal regression model using the the ` Mass ` package and function ` polr `where $-\beta=\eta$ using the same parameters as final model $lm_\omega$ @Modern :
...
For Starcraft II players this ranked season, for everyone one unit increase in players APM the odds of being ranked plat below is decrease 3.3% holding all other variables constant.

```{r, error=FALSE, echo=FALSE,collapse=TRUE}
require(foreign)
require(ggplot2)
require(MASS)
require(Hmisc)
...
co <-exp(coef(m))
c<-round(cbind(ci,Nominal=co),3)

kable(ctable,format = "markdown", digits = 4, caption="Ordinal Model" )
kable(c,caption="Exp. Confidence Intervals")

#https://stats.idre.ucla.edu/r/dae/ordinal-logistic-regression/
#The intercept term shows cumulative odds
#melt, group by , summarize
```
To generate bias P-Values a z-test is used where the t-value is compared with the standard normal distribution. These p-values have decreasing bias as sample size increases and considering $n=$`r nrow(sc)` the bias should be reasonably diminished producing subtlety asymmetric confidence intervals. 

# Ideas 

- boxcox plot for log transform?
- Contour plot , drill down with a leave-oneout df dfbeta unsual observation in leave one out until
- normality of the residual (without them)
- fitted vs error (Definite trend favoring the higher observations)
- partial residuals by each predictor Lecture - (Model structure and gooodness of fit and for multiple modality)
