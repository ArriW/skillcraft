...
git add --all
git commit -m "flattening predictors"
git push
```
...
ggplot(sc_lm_final, aes(x=APM, y=factor(LeagueIndex),color='blue'))+geom_point() +geom_point(aes(y=round(sc_lm_rounded$fitted.values),color='red')) +ggtitle("Fitted Values Rounded and Sample by \nAPM")+theme_classic()

round(sum((sc$LeagueIndex==round(sc_lm_rounded$fitted.values)))/count(sc),2)
```
