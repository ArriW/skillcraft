...
The appendix[Make hyperlink], covers each in depth but the following are highlighted because their used in the final analysis.

**LeagueIndex** is was covered thoroughy in the above section.

**Perception Action Cycles (PACs)** are the circular flow of information between and organism and its environment in a sensor-guided sequence of behavior towards a goal[ISBN 978-1-4419-1452-1]. Each action the organism takes causes a change in the environment that is processed subsequently leads to action-information iterration in persuit of the goal.

In this context this dataset PACs are aggregiate of screen movements where each PAC is a screen fixication of at least one action [https://archive.ics.uci.edu/ml/datasets/SkillCraft1+Master+Table+Dataset]. 

The some of the time averaged metrics are per SC2 timestamps while other are per milisecond. To make these metrics more interperatable each metric will be converted into seconds. There are roughly 88.5 timestamps per second so each metrix in timestamps will be divided by _____. [https://archive.ics.uci.edu/ml/datasets/SkillCraft1+Master+Table+Dataset].
