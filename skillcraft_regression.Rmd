...
**AssignToHotkeys and APM** have the highlest correlation within the final model. **AssignToHotkeys** was considered to be dropped along with the PAC related predictors prior but I imaged the preidcotr added a significant flavor to what type of actions regardless of its potential for confounding.**AssignToHotkeys** in game is when a player assigns units or buildings to hotkeys. For example within a match if the player has two armies they may select all of the units in army 1 and use the hotkey combination *CTRL+1* to assign those units to hotkey *1* for future rapid selection. Then the same player can select their second army use *Ctrl+2* to hotkey *2*. This works for any commandable unit or building in game, allowing the player to on the fly reshape hotkeys based as assets are gained or lost. 

Both variables will be removed from the model one at a time and then compared to the final model that contains both. In both subset models' **RSS** are statistically significant differnces. In terms of the impact to $adjR^2$, as **APMs** seem to a explain a significant amount more than **AssignToHotkeys** as the models have with $adjR^2$ of `r round(summary(sc_lm_6)[["adj.r.squared"]],2)` and `r round(summary(sc_lm_7)[["adj.r.squared"]],2)` compared to the final model `r round(summary(sc_lm_final)[["adj.r.squared"]],2)`. To reaffirm the initial effort of dropping predictor for the final model persued in the next model.
...
Without using cross validation we can see some basic issues with the fitted values of the each model reviewed. The first plot is a scatterplot with the fitted values and the dominate predictor APM. Some of the players are excepted to have a LeagueIndex>>10 which does not exist in sampled response. Although LeagueIndex > 7 does tease that Grandmaster tier is unbounded tier in terms of skillpoints. Overall, the the continous fitted values fail to capture the discrete nature of the response. 

To help interpret the result the fitted values will be flattened such that  when LeagueIndex > 7 will be set LeagueIndex=7 then each value will be rounded so only integer values remain.
...
p1<-ggplot(sc_lm_final, aes(x=APM, y=factor(LeagueIndex),color='blue'))+geom_point() +geom_point(aes(y=fitted(sc_lm_final),color='red')) +ggtitle("Fitted Values and Sample by \nAPM")+theme_classic()

p2<-ggplot(sc_lm_final, aes(x=APM, y=factor(LeagueIndex),color='blue'))+geom_point() +geom_point(aes(y=round(sc_lm_rounded$fitted.values),color='red')) +ggtitle("Fitted Values Rounded and Sample by \nAPM")+theme_classic()

grid.arrange(p1,p2,ncol=2)
```

To assess the predictive accuracy of the model the flattened and rounded values fitted values are then compared to the sampled values overall providing a `r round(sum((sc$LeagueIndex==round(sc_lm_rounded$fitted.values)))/count(sc),2)*100`% accuracy. This is a very low accuracy considering the 100% of the data was used to train the model. Another part of the models bias is that it does not place anyone below **LeagueIndex=** `r min(round(fitted(sc_lm)))`. The following shows the predictive accuracy at each LeagueIndex.
...
ggplot(sc_predicted_agg,aes(x=LeagueIndex,y=correct_perc))+geom_col(fill='darkgreen')+ylim(0,1)+
geom_text(aes(label = paste("n =",n)), vjust = -0.5)+theme_classic()+ylab("Correct %")
```
