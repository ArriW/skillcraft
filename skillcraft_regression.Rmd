...
git add --all
git commit -m "Editting. Removing overkill for midterm 2. Touching up the prelude to final. Pre-model confounding."
git push
```
...
To compare  Performing an ANOVA test we find that there is a significant difference in the models (see table below), but the $adjR^2$ is barely different. Starting at `r round(summary(sc_lm)[["adj.r.squared"]],2)` and ending at `r round(summary(sc_lm_4)[["adj.r.squared"]],2)`.
